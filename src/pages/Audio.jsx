...
export default function Audio() {
  ...
  const seo = useSEO("audio");

  const audioCategories = [
    { name: "Headphones", icon: <Headphones className="w-5 h-5" /> },
    { name: "Speakers", icon: <Speaker className="w-5 h-5" /> },
    { name: "Earbuds", icon: <Ear className="w-5 h-5" /> },
    { name: "Microphones", icon: <Mic className="w-5 h-5" /> },
    { name: "Soundbars", icon: <Music className="w-5 h-5" /> },
    { name: "Home Audio", icon: <Home className="w-5 h-5" /> },
    { name: "Bluetooth", icon: <Bluetooth className="w-5 h-5" /> },
    { name: "Amplifiers", icon: <Volume2 className="w-5 h-5" /> },
  ];
    ...
    const fetchAudio = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const audio = productsArray.filter(product =>
            product.category &&
            (product.category.toLowerCase().includes("audio") ||
            product.category.toLowerCase().includes("sound") ||
            ["headphone", "headset", "earphone", "earbud", "speaker", "bluetooth speaker", "soundbar", "microphone", "mic", "amplifier", "receiver", "subwoofer", "woofer", "tweeter", "headset", "headphone", "headphones", "earphones", "earbuds", "wireless", "bluetooth", "sound system", "home theater", "studio monitor"].some(keyword =>
          ...
          setAudioProducts(audio);
          setDisplayProducts(audio.slice(0, 6));
        } else {
          setMessage(data.message || "Failed to load audio products.");
        }
      } catch (err) {
        setMessage("Error loading audio products.");
  ...
  useEffect(() => {
    if (showAll) setDisplayProducts(audioProducts);
    else setDisplayProducts(audioProducts.slice(0, 6));
  }, [showAll, audioProducts]);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-gray-700 text-base sm:text-lg max-w-2xl mx-auto">
              Discover immersive sound experiences with our curated selection of headphones, speakers, and audio equipment.
            </p>
          </motion.div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 sm:mb-8 gap-3">
              <div className="text-center sm:text-left">
                <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">
                  Featured Audio Devices
                </h2>
                <p className="text-gray-600 mt-1 text-sm sm:text-base">
                  {showAll
                    ? `Showing all ${audioProducts.length} audio devices`
                    : `Showing ${displayProducts.length} of ${audioProducts.length} audio devices`}
                </p>
              </div>

              {!showAll && audioProducts.length > 6 && (
                <motion.button
            ...
            {displayProducts.length === 0 ? (
              ...
              >
                ...
                <p className="text-gray-600 text-sm sm:text-base">
                  Check back soon for new audio products!
                </p>
              </motion.div>
            ) : (
              <>
          ...
          >
            <h3 className="text-xl sm:text-2xl font-bold mb-3">
              Need Audio Recommendations?
            </h3>
            <p className="mb-6 max-w-2xl mx-auto opacity-90 text-sm sm:text-base">
              Our audio experts can help you find the perfect sound equipment for your needs, whether for home, studio, or on-the-go.
            </p>

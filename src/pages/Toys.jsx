...
export default function Toys() {
  ...
  // Toys categories
  const categories = [
    { value: "all", label: "All Toys", icon: <Sparkle className="w-4 h-4" /> },
    {
      value: "educational",
      label: "Educational Toys",
      icon: <BookOpen className="w-4 h-4" />,
    },
    { value: "rc", label: "RC Toys", icon: <Car className="w-4 h-4" /> },
    {
      value: "robotic",
      label: "Robotic Toys",
      icon: <Cpu className="w-4 h-4" />,
    },
    { value: "stem", label: "STEM Toys", icon: <Atom className="w-4 h-4" /> },
    {
      value: "musical",
      label: "Musical Toys",
  ...
  const concerns = [
    ...
    { value: "kids", label: "Kids (4–7)", icon: <Smile className="w-4 h-4" /> },
    {
      value: "preteens",
      label: "Pre-Teens (8–12)",
      icon: <Users className="w-4 h-4" />,
    },
    { value: "teens", label: "Teens (13+)", icon: <Zap className="w-4 h-4" /> },
    {
      value: "learning",
  ...
  const brands = [
    ...
    { value: "fisherprice", label: "Fisher-Price" },
    { value: "vtech", label: "VTech" },
    { value: "hasbro", label: "Hasbro" },
    { value: "nerf", label: "NERF" },
    { value: "xiaomi", label: "Xiaomi (Mi Toys)" },
    { value: "dji", label: "DJI (Drones)" },
    { value: "wowwee", label: "WowWee" },
    { value: "smartivity", label: "Smartivity" },
  ];

  // Price ranges (toys products typically have different ranges)
  const priceRanges = [
    ...
    const fetchToysProducts = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setFilteredProducts(toysProducts);
        } else {
          setMessage(data.message || "Failed to load toys products.");
        }
      } catch (err) {
        setMessage("Error loading toys products.");
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== "all") {
      filtered = filtered.filter((product) => {
        ...
        if (selectedCategory === "educational") {
        ...
        } else if (selectedCategory === "rc") {
          const rcKeywords = ["rc", "remote", "remote control", "radio control"];
          return rcKeywords.some(
            (keyword) =>
              category.includes(keyword) ||
              subCategory.includes(keyword) ||
              tags.includes(keyword) ||
        ...
        } else if (selectedCategory === "robotic") {
          const roboticKeywords = ["robot", "robotic", "robotics"];
          return roboticKeywords.some(
            (keyword) =>
              category.includes(keyword) ||
              subCategory.includes(keyword) ||
              tags.includes(keyword) ||
          ...
          const stemKeywords = ["stem", "science", "technology", "engineering", "math"];
          return stemKeywords.some(
            (keyword) =>
              category.includes(keyword) ||
              subCategory.includes(keyword) ||
              tags.includes(keyword) ||
        ...
        } else if (selectedCategory === "musical") {
          const musicalKeywords = ["music", "musical", "instrument", "sound"];
          return musicalKeywords.some(
            (keyword) =>
              category.includes(keyword) ||
              subCategory.includes(keyword) ||
              tags.includes(keyword) ||
          ...
          const gamingKeywords = ["game", "gaming", "console", "video game"];
          ...
          const outdoorKeywords = ["outdoor", "sports", "playground", "park"];
    ...
    // Concern filter (Age groups and development areas)
    if (selectedConcern !== "all") {
      filtered = filtered.filter((product) => {
        ...
        if (selectedConcern === "toddlers") {
          const toddlerKeywords = ["toddler", "1-3", "baby", "infant", "early learning"];
          return toddlerKeywords.some(
            (keyword) =>
              description.includes(keyword) ||
              name.includes(keyword) ||
              tags.includes(keyword)
          );
        } else if (selectedConcern === "kids") {
          const kidsKeywords = ["kids", "child", "4-7", "preschool", "kindergarten"];
          return kidsKeywords.some(
            (keyword) =>
              description.includes(keyword) ||
              name.includes(keyword) ||
              tags.includes(keyword)
          );
        } else if (selectedConcern === "preteens") {
          const preteenKeywords = ["preteen", "8-12", "tween", "school age"];
          return preteenKeywords.some(
            (keyword) =>
              description.includes(keyword) ||
              name.includes(keyword) ||
              tags.includes(keyword)
          );
        } else if (selectedConcern === "teens") {
          const teenKeywords = ["teen", "13+", "adolescent", "teenager"];
          return teenKeywords.some(
            (keyword) =>
              description.includes(keyword) ||
              name.includes(keyword) ||
              tags.includes(keyword)
          );
        } else if (selectedConcern === "learning") {
          ...
          return learningKeywords.some(
            (keyword) =>
              description.includes(keyword) ||
              name.includes(keyword) ||
              tags.includes(keyword)
          );
        } else if (selectedConcern === "creativity") {
          ...
          return creativityKeywords.some(
            (keyword) =>
              description.includes(keyword) ||
              name.includes(keyword) ||
              tags.includes(keyword)
          );
        } else if (selectedConcern === "motor") {
          const motorKeywords = ["motor", "skills", "physical", "coordination", "hand-eye"];
          return motorKeywords.some(
            (keyword) =>
              description.includes(keyword) ||
              name.includes(keyword) ||
              tags.includes(keyword)
        ...
        return (
          description.includes(selectedConcern) ||
    ...
    if (toyType !== "all") {
      filtered = filtered.filter((product) => {
        ...
        if (toyType === "battery") {
          const batteryKeywords = ["battery", "batteries", "aa", "aaa"];
          return batteryKeywords.some(
            (keyword) =>
              description.includes(keyword) ||
              name.includes(keyword) ||
              tags.includes(keyword)
          );
        } else if (toyType === "rechargeable") {
          const rechargeableKeywords = ["rechargeable", "recharge", "charging", "lithium"];
          return rechargeableKeywords.some(
            (keyword) =>
              description.includes(keyword) ||
              name.includes(keyword) ||
              tags.includes(keyword)
          );
        } else if (toyType === "remote") {
          const remoteKeywords = ["remote", "rc", "remote control"];
        ...
        } else if (toyType === "appcontrolled") {
          const appKeywords = ["app", "application", "smartphone", "bluetooth", "wifi"];
        ...
        } else if (toyType === "voice") {
          const voiceKeywords = ["voice", "voice control", "voice activated", "alexa", "google"];
        ...
        return (
          description.includes(toyType) ||
          tags.includes(toyType) ||
          product.type?.toLowerCase().includes(toyType)
        );
      });
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/20 to-purple-50/20 flex items-center justify-center p-4">
        <div className="text-center max-w-md w-full">
          <AlertCircle className="w-12 h-12 md:w-16 md:h-16 text-blue-500 mx-auto mb-3 md:mb-4" />
          <p className="text-blue-500 font-semibold text-base md:text-lg mb-3 md:mb-4">
            {message}
          </p>
          <button
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <div className="inline-flex items-center gap-2 mb-4">
              ...
              <span className="text-blue-600 font-medium">Toys Collection</span>
            </div>
            ...
            <p className="text-gray-700 max-w-2xl mx-auto text-base md:text-lg">
              Discover toys and games for all ages
            </p>
          </motion.div>
              ...
              },
              {
                ...
                value:
                  products.filter(
                    (p) =>
                      p.tags?.some(tag => 
                        ["educational", "learning", "stem", "science"].includes(tag.toLowerCase())
                      )
                  ).length || Math.floor(products.length * 0.4),
                            ...
                            <div className="flex flex-wrap gap-2">
                              {concerns.map((concern) => (
                                <button
                                  key={concern.value}
                                  onClick={() =>
                                    setSelectedConcern(concern.value)
                                  }
                                  className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-all ${
          ...
          {(searchTerm ||
            ...
            >
              <div className="flex flex-wrap gap-2 items-center">
                ...
                {toyType !== "all" && (
                  <span className="px-3 py-1.5 bg-blue-100 text-blue-700 rounded-lg text-sm flex items-center gap-1">
                    {toyTypes.find((s) => s.value === toyType)?.label}
                    <button
                      onClick={() => setToyType("all")}
                      className="hover:text-blue-900 p-1 rounded-full hover:bg-blue-50"
                    >
                      <X className="w-3 h-3" />
                    </button>
            ...
            >
              ...
              <p className="text-gray-600 mb-6 px-4">
                ...
                selectedConcern !== "all"
                  ...
                  : "No toys products available at the moment."}
              </p>
              {(searchTerm ||
                selectedCategory !== "all" ||
                selectedConcern !== "all") && (
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                  <div>
                    <h3 className="text-xl font-bold text-gray-900 mb-2">
                      Toy Tips & Buying Guides
                    </h3>
                    <p className="text-gray-700">
                      Learn how to choose the best toys for different age groups and developmental stages.
                    </p>
                  </div>

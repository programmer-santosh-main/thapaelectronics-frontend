...
export default function Automotive() {
  ...
  const seo = useSEO("automotive");

  // Automotive categories
  const automotiveCategories = [
    { value: "all", label: "All Products", icon: <Car className="w-4 h-4" /> },
    { value: "audio", label: "Car Audio", icon: <Radio className="w-4 h-4" /> },
    { value: "charging", label: "Chargers", icon: <Battery className="w-4 h-4" /> },
    { value: "camera", label: "Dash Cams", icon: <Camera className="w-4 h-4" /> },
    { value: "navigation", label: "GPS Navigation", icon: <Navigation className="w-4 h-4" /> },
    { value: "safety", label: "Safety & Security", icon: <Shield className="w-4 h-4" /> },
    { value: "tools", label: "Tools & Maintenance", icon: <Activity className="w-4 h-4" /> },
    { value: "accessories", label: "Accessories", icon: <Sparkles className="w-4 h-4" /> },
  ];

  // Product types
  const productTypes = [
    { value: "all", label: "All Types", icon: <Tag className="w-4 h-4" /> },
    { value: "speakers", label: "Speakers", icon: <Radio className="w-4 h-4" /> },
    { value: "chargers", label: "Chargers", icon: <Battery className="w-4 h-4" /> },
    { value: "dashcams", label: "Dash Cams", icon: <Camera className="w-4 h-4" /> },
    { value: "gps", label: "GPS Devices", icon: <Navigation className="w-4 h-4" /> },
    { value: "alarms", label: "Alarms & Security", icon: <Shield className="w-4 h-4" /> },
    { value: "tools", label: "Car Tools", icon: <Activity className="w-4 h-4" /> },
    { value: "holders", label: "Phone Holders", icon: <Car className="w-4 h-4" /> },
  ];
  ...
  const brands = [
    ...
    { value: "viofo", label: "Viofo" },
    { value: "70mai", label: "70mai" },
    { value: "bosch", label: "Bosch" },
    { value: "blackvue", label: "BlackVue" },
  ];

  // Price ranges for automotive electronics
  const priceRanges = [
    { min: 0, max: 1000, label: "Under रू 1,000" },
    { min: 1000, max: 3000, label: "रू 1,000 - 3,000" },
    { min: 3000, max: 8000, label: "रू 3,000 - 8,000" },
    { min: 8000, max: 20000, label: "रू 8,000 - 20,000" },
    { min: 20000, max: 50000, label: "Over रू 20,000" },
  ];
  ...
  useEffect(() => {
    const fetchAutomotiveProducts = async () => {
      try {
        setLoading(true);
        ...
        if (res.ok) {
          ...
          // Filter automotive electronics products
          const automotiveProducts = productsArray.filter((product) => {
            ...
            return automotiveKeywords.some(
              (keyword) =>
          ...
          setFilteredProducts(automotiveProducts);
        } else {
          setMessage(data.message || "Failed to load automotive products.");
        }
      } catch (err) {
        setMessage("Error loading automotive products.");
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== "all") {
      filtered = filtered.filter((product) => {
        ...
        if (selectedCategory === "audio") {
          ...
          return audioKeywords.some(
            (keyword) =>
              category.includes(keyword) ||
              name.includes(keyword) ||
              tags.includes(keyword)
          );
        } else if (selectedCategory === "charging") {
          ...
          const navKeywords = ["gps", "navigation", "navigator", "map"];
    ...
    if (selectedType !== "all") {
      filtered = filtered.filter((product) => {
        ...
        if (selectedType === "speakers") {
          return (
            category.includes("speaker") ||
            name.includes("speaker") ||
            tags.includes("speaker")
          );
        } else if (selectedType === "chargers") {
          return (
            category.includes("charger") ||
            name.includes("charger") ||
            tags.includes("charger")
          );
        } else if (selectedType === "dashcams") {
          return (
            category.includes("dashcam") ||
            name.includes("dashcam") ||
            tags.includes("dashcam") ||
            category.includes("dash cam") ||
            name.includes("dash cam")
          );
        }
  ...
  const refreshProducts = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const automotiveProducts = productsArray.filter((product) => {
          const category = product.category?.toLowerCase();
          const keywords = ["car", "automotive", "auto", "vehicle", "dashcam", "gps"];
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-gray-700 max-w-2xl mx-auto text-base md:text-lg">
              Upgrade your vehicle with premium car electronics, accessories, and safety gadgets
            </p>
          </motion.div>
          ...
          {isMobile && (
            <div className="mb-6">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search dash cams, car audio, GPS..."
                  value={searchTerm}
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
                  <div className="flex-1 relative">
                    ...
                    <input
                      type="text"
                      placeholder="Search by product, brand, or car accessory..."
                      value={searchTerm}
                      onChange={(e) => setSearchTerm(e.target.value)}
            ...
            >
              <div className="flex flex-wrap gap-2 items-center">
                ...
                {selectedCategory !== "all" && (
                  <span className="px-3 py-1.5 bg-orange-100 text-orange-700 rounded-lg text-sm flex items-center gap-1">
                    {
                      automotiveCategories.find((c) => c.value === selectedCategory)
                        ?.label
                    }
                    <button
                ...
                {selectedType !== "all" && (
                  <span className="px-3 py-1.5 bg-red-100 text-red-700 rounded-lg text-sm flex items-center gap-1">
                    {
                      productTypes.find((t) => t.value === selectedType)
                        ?.label
                    }
                    <button
            ...
            >
              ...
              <p className="text-gray-600 mb-6 px-4">
                {searchTerm || selectedCategory !== "all" || selectedBrand !== "all"
                  ...
                  : "No automotive electronics available at the moment."}
              </p>
              {(searchTerm || selectedCategory !== "all" || selectedBrand !== "all") && (
                <button
                  onClick={resetFilters}
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                  <div>
                    <h3 className="text-xl font-bold text-gray-900 mb-2">
                      Car Electronics Guide
                    </h3>
                    <p className="text-gray-700">
                      Get expert advice on choosing the right car accessories, installation tips, and maintenance guides.
                    </p>
                  </div>
                  <div className="flex gap-3">
                    <button
                      onClick={() => navigate("/blog/car-electronics")}
